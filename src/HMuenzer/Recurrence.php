...
class Recurrence
{
    ...
    protected function read_options($options = array())
    {
        ...
        if ($options['rrule']) {
            foreach (explode(";", $options['rrule']) as $part) {      //parse RRULE if present
                list($rule, $value) = explode("=", $part);
                $rule = strtolower($rule);
                if (property_exists($this, $rule) AND !array_key_exists($rule, $options)
                )       //ignore unknown properties, manually options precede
                {
                    $options[$rule] = $value;
                }
            }
    ...
    protected function evaluate_options()
    {
        ...
        if (null !== $this->dtend) {
            ...
            if (array("00", "00", "00") == $this->explode("H-i-s", $this->dtstart) AND array(
                    "00",
                    "00",
                    "00"
        ...
        switch ($this->freq) {
            case "YEARLY":
                $this->expansions = array(
                    array('bymonth'),
                    array('byweekno'),
                    array('byyearday', 'bymonthday', 'byday'),
                    array('byhour'),
                    array('byminute'),
                    array('bysecond')
                );
    ...
    public function next()
    {
        if ($this->error) {
            return false;
        }                                                 //property error: abort
        if ($this->count AND $this->current_count >= $this->count) {
            return false;
        }        //max count reached: abort
        if ($this->cached_dates) {
            return $this->output();
        }                                //return cached dates first
        if ($this->before AND $this->current_date >= $this->before) {                    //interval out of range
            if ($this->cached_rdates) {
                return $this->output();
            }                               //rdates left: output
            return false;                                                                  //abort
        }
        ...
        do {
            ...
            if (++$safety_brake > 1000) {
                break;
            }                                              //fail safe: abort after 1000 failed iterations

            if (1 < $this->iteration) {
                $this->next_interval();
            }                               //apply interval from 2nd iteration on (dtstart is first date to be expanded)
            if ($this->out_of_range()) {
                return false;
            }                                        //current date out of range: abort
            if ($this->skip_not_in_range AND $this->not_in_range()) {
                continue;
            }               //current date not in range: next interval
            ...
            if (!$this->expansion_count) {                                                   //no EXPANSIONS, apply LIMITATIONS only
                if ($this->exdate AND in_array($this->current_date, $this->exdate)) {
                    continue;
                }   //restricted by exdate: next interval
                foreach ($this->limitations as $limitation) {
                    if ($this->$limitation AND false == $this->{'limit_' . $limitation}($this->current_date)) {
                        continue 2;
                    }
                ...
                foreach ($dates as $date) {                                                    //LIMITATIONS
                    foreach ($this->limitations as $limitation) {
                        if ($this->$limitation AND false == $this->{'limit_' . $limitation}($date)) {
                            continue 2;
                        }
                ...
                $limited_dates = array();
                foreach ($dates as $date) {
                    if (null !== $this->before AND $date > $this->before) {
                        continue;
                    }               //check range
        ...
        switch ($this->freq) {
            ...
            case "MONTHLY":
                ...
                elseif ($this->bymonthday OR $this->byday)                                                //day part is defined by rule:
                {
                    ...
                    $n = 1;
                    while ($d > $this->date("t", $this->mktime(0, 0, 0, $m + $this->interval * $n, 1,
                            $Y))) {      //skip months with fewer days
                        $n++;
                        $this->iteration++;
                    }
    ...
    protected function out_of_range()
    {
        ...
        if (!$this->expansion_count AND $this->current_date > $this->before) {
            return true;
        ...
        switch ($this->freq) {                                                                         //check range of possible expansions
            case "YEARLY":
                ...
                if ($this->byweekno AND $year_details['week_start'] < $year_details['start']) {
    ...
    protected function not_in_range()
    {
        ...
        if (!$this->expansion_count AND $this->current_date < $this->after) {
            return true;
        }           //no expansions: check current date only
        ...
        switch ($this->freq) {                                                                      //check range of possible expansions
            case "YEARLY":
                $year_details = $this->year_details($this->date("Y", $this->current_date));
                if ($this->byweekno AND $year_details['week_end'] > $year_details['end']) {
    ...
    protected function expand_bymonth($date)
    {
        ...
        if ($this->byweekno OR $this->byyearday OR $this->bymonthday OR $this->byday) {
            $shift = true;
    ...
    protected function expand_byweekno($date)
    {
        ...
        if (!$this->byyearday AND !$this->bymonthday AND !$this->byday) {                 //day part NOT defined by rule: take weekday from dtstart
            ...
            $pos_weekday = array_search($weekday,
                $this->wkst_seq);                          //position of the weekday (depends on wkst)
        }
        ...
        foreach ($year as $Y => $year_details) {
            foreach ($this->byweekno as $week) {                                              //create one date for each WEEK
                ...
                if (isset($pos_weekday)) {                                                        //weekday is specified by dtstart: test weekday only
                    $test_date = $this->mktime($H, $i, $s, 1,
                        1 + $year_details['week_offset'] + $pos_weekday + ($week - 1) * 7, $Y);
                    if ($test_date < $start) {
                        continue;
                    ...
                    do {
                        ...
                        $test_date = $this->mktime($H, $i, $s, 1,
                            1 + $year_details['week_offset'] + $pos + ($week - 1) * 7, $Y);
                        if ($test_date >= $start AND $test_date <= $end) {
                            break;
                        }
                        $pos++;
                    } while (1);
    ...
    protected function expand_byyearday($date)
    {
        ...
        $start = $this->mktime(0, 0, 0, 1, 1,
            $Y);                                                   //results are limited to current year
    ...
    protected function expand_byday($date)
    {
        ...
        if ("WEEKLY" == $this->freq OR $this->byweekno) {                                         //special expand for WEEKLY
            ...
            foreach ($this->byday as $option) {                                                       //apply BYDAY
                ...
                $pos = array_search($option['weekday'],
                    $this->wkst_seq);                                //position of day from week start
                $date = $this->mktime($H, $i, $s, $m, $d + $pos, $Y);
                if (isset($start) AND $date < $start) {
                    continue;
                }                                          //out of range: skip day
                if (isset($end) AND $date > $end) {
                    continue;
                }
                $dates[] = $date;
            }
        ...
        if ("MONTHLY" == $this->freq OR $this->bymonth) {                                         //special expand for MONTHLY
            ...
            foreach ($this->byday as $option) {                                                       //apply BYDAY
                $pos = array_search($option['weekday'],
                    $month_details['start_seq']);                  //position of day from month start
                $all_days = array();
        ...
        if ("YEARLY" == $this->freq) {                                                            //special expand for YEARLY
            ...
            foreach ($this->byday as $option) {                                                       //apply BYDAY
                $pos = array_search($option['weekday'],
                    $year_details['start_seq']);                   //position of day from year start
                $all_days = array();
    ...
    {
        if (in_array($this->date("n", $date), $this->bymonth)) {
            return true;
        }
        return false;
    }
    ...
    protected function limit_byyearday($date)
    {
        ...
        if (in_array($z - ($L ? 366 : 365), $this->byyearday)) {
            return true;
        }
        return false;
    }
    ...
    {
        if (in_array($this->date("G", $date), $this->byhour)) {
            return true;
        }
        return false;
    }
    ...
    {
        if (in_array((int)$this->date("i", $date), $this->byminute)) {
            return true;
        }
        return false;
    }
    ...
    protected function month_details($month, $year)
    {
        ...
        return $this->cached_details['month'][$year][$month] = compact(array(
            'start',
            'end',
            'start_seq',
            'number_of_days'
    ...
    protected function year_details($year)
    {
        ...
        return $this->cached_details['year'][$year] = compact(array(
            'start',
            'end',
            'start_seq',
            'number_of_days',
